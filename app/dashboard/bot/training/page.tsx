import { resolveCurrentOrganizationId } from '@/lib/current-organization'

import TrainingDataClient from './TrainingDataClient'
import { requireAuthUserId } from '@/lib/auth-server'
import { prisma } from '@/lib/prisma'
import type { Bot } from '../interactions/action'
import { DashboardPageHeader } from '@/components/dashboard-page-header'

export default async function BotTrainingPage() {
  const userId = await requireAuthUserId()
  const organizationId = await resolveCurrentOrganizationId({ userId })
  if (!organizationId) {
    console.error('Organization not found!')
    return (
      <DashboardPageHeader
        title='Training Data'
        description='Teach your bot with websites and documents so it can answer with your content.'>
        <div className='alert-danger'>Organization not found!</div>
      </DashboardPageHeader>
    )
  }

  const botsRaw = await prisma.bots.findMany({
    where: { organizationId },
  })
  const bots: Bot[] = botsRaw.map(b => ({
    id: b.id,
    organization_id: b.organizationId ?? organizationId,
    name: b.name,
    institute_name: b.instituteName ?? null,
    tone: b.tone ?? null,
    role: b.role ?? null,
    business_description: b.businessDescription ?? null,
    first_message: b.firstMessage ?? null,
    confirmation_message: b.confirmationMessage ?? null,
    lead_capture_message: b.leadCaptureMessage ?? null,
    capture_leads: b.captureLeads,
    lead_capture_timing: (b.leadCaptureTiming ??
      null) as Bot['lead_capture_timing'],
    capture_name: Boolean(b.captureName),
    capture_email: Boolean(b.captureEmail),
    capture_phone: Boolean(b.capturePhone),
    created_at: b.createdAt.toISOString(),
    updated_at: b.updatedAt.toISOString(),
  }))

  return (
    <DashboardPageHeader
      title='Training Data'
      description='Teach your bot with websites and documents so it can answer with your content.'>
      <TrainingDataClient bots={bots} />
    </DashboardPageHeader>
  )
}
